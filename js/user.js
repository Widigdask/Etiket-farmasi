...
let pasienData = [];
    ...
    const PRINT_QUEUE_PAGE_SIZE = 5;
    let labelTypographyFrame = null;
let labelTypographySettings = {
  topScale: 100,
  middleScale: 100,
  bottomScale: 100,
  family: "default",
  bold: false,
    ...
    const STORAGE_KEYS = {
      ...
      typography: "farmasi_etiket_typography_settings_v1"
    };
    ...
    function loadPersistentData() {
      ...
      labelTypographySettings = normalizeLabelTypographySettings(loadFromStorage(STORAGE_KEYS.typography, {}));
      applyInstansiSettingsToForm();
      syncLabelTypographyControls();
    }
    ...
    function renderAll() {
      ...
      renderPrintQueue();
      syncPrintModeUI();
      if (typeof renderAkunTable === 'function') renderAkunTable();
    }
    ...
    function normalizeLabelTypographySettings(settings) {
      const allowedFamilies = ["default", "arial", "roboto", "times", "monospace"];
  const legacyScale = parseInt(settings && settings.scale, 10);
  const parsedTopScale = parseInt(settings && settings.topScale, 10);
  const parsedMiddleScale = parseInt(settings && settings.middleScale, 10);
  const parsedBottomScale = parseInt(settings && settings.bottomScale, 10);
  const fallbackScale = Number.isFinite(legacyScale) ? legacyScale : 100;
  const bottomFallbackScale = Number.isFinite(parsedBottomScale) ? parsedBottomScale : fallbackScale;
  return {
    topScale: Math.min(150, Math.max(70, Number.isFinite(parsedTopScale) ? parsedTopScale : fallbackScale)),
    middleScale: Math.min(150, Math.max(70, Number.isFinite(parsedMiddleScale) ? parsedMiddleScale : bottomFallbackScale)),
    bottomScale: Math.min(150, Math.max(70, bottomFallbackScale)),
    family: allowedFamilies.includes(settings && settings.family) ? settings.family : "default",
        bold: !!(settings && settings.bold),
        italic: !!(settings && settings.italic),
        underline: !!(settings && settings.underline)
      };
    }

    function syncLabelTypographyControls() {
  const topSizeInput = document.getElementById("label-font-size-top");
  const topSizeValue = document.getElementById("label-font-size-top-value");
  const middleSizeInput = document.getElementById("label-font-size-middle");
  const middleSizeValue = document.getElementById("label-font-size-middle-value");
  const bottomSizeInput = document.getElementById("label-font-size-bottom");
  const bottomSizeValue = document.getElementById("label-font-size-bottom-value");
      const familyInput = document.getElementById("label-font-family");
      const boldInput = document.getElementById("label-font-bold");
      const italicInput = document.getElementById("label-font-italic");
      const underlineInput = document.getElementById("label-font-underline");
  if (topSizeInput) topSizeInput.value = labelTypographySettings.topScale;
  if (topSizeValue) topSizeValue.textContent = labelTypographySettings.topScale + "%";
  if (middleSizeInput) middleSizeInput.value = labelTypographySettings.middleScale;
  if (middleSizeValue) middleSizeValue.textContent = labelTypographySettings.middleScale + "%";
  if (bottomSizeInput) bottomSizeInput.value = labelTypographySettings.bottomScale;
      if (bottomSizeValue) bottomSizeValue.textContent = labelTypographySettings.bottomScale + "%";
      if (familyInput) familyInput.value = labelTypographySettings.family;
      if (boldInput) boldInput.checked = labelTypographySettings.bold;
      if (italicInput) italicInput.checked = labelTypographySettings.italic;
      if (underlineInput) underlineInput.checked = labelTypographySettings.underline;
    }

    function updateLabelTypography() {
  labelTypographySettings = normalizeLabelTypographySettings({
    topScale: document.getElementById("label-font-size-top")?.value,
    middleScale: document.getElementById("label-font-size-middle")?.value,
    bottomScale: document.getElementById("label-font-size-bottom")?.value,
        family: document.getElementById("label-font-family")?.value,
        bold: document.getElementById("label-font-bold")?.checked,
        italic: document.getElementById("label-font-italic")?.checked,
        underline: document.getElementById("label-font-underline")?.checked
      });
      saveToStorage(STORAGE_KEYS.typography, labelTypographySettings);
      syncLabelTypographyControls();
      updatePreview();
    }

    function resetLabelTypography() {
      labelTypographySettings = normalizeLabelTypographySettings({});
      saveToStorage(STORAGE_KEYS.typography, labelTypographySettings);
      syncLabelTypographyControls();
      updatePreview();
    ...
    function getPrintableLabelAreas() {
      return [
        document.getElementById("printable-area-landscape"),
        document.getElementById("printable-area-portrait"),
        document.getElementById("printable-area-minum-rs")
      ].filter(Boolean);
    }

    function clearLabelTypographyFontScale() {
      document.querySelectorAll("[data-label-original-font-size]").forEach((el) => {
        const originalSize = el.dataset.labelOriginalFontSize;
        const originalPriority = el.dataset.labelOriginalFontPriority || "";
        if (originalSize) el.style.setProperty("font-size", originalSize, originalPriority);
        else el.style.removeProperty("font-size");
        delete el.dataset.labelOriginalFontSize;
        delete el.dataset.labelOriginalFontPriority;
      });
    }
...
function getLabelTypographyScale(el) {
  const topSelectors = [
        ".l-head",
        ".lbl-rs-header",
        ".minum-header",
  ...
  if (el.closest(topSelectors)) return labelTypographySettings.topScale / 100;
  if (el.closest(bottomSelectors)) return labelTypographySettings.bottomScale / 100;
  return labelTypographySettings.middleScale / 100;
}

    function applyLabelTypography() {
      clearLabelTypographyFontScale();

      getPrintableLabelAreas().forEach((root) => {
        root.dataset.labelFontFamily = labelTypographySettings.family;
        root.classList.toggle("label-user-bold", labelTypographySettings.bold);
        root.classList.toggle("label-user-italic", labelTypographySettings.italic);
        root.classList.toggle("label-user-underline", labelTypographySettings.underline);

        [root, ...root.querySelectorAll("*")].forEach((el) => {
          if (!elementHasOwnText(el) || el.closest("svg")) return;
          const scale = getLabelTypographyScale(el);
          if (scale === 1) return;
          const computedSize = parseFloat(getComputedStyle(el).fontSize);
          if (!Number.isFinite(computedSize)) return;
          el.dataset.labelOriginalFontSize = el.style.getPropertyValue("font-size");
          el.dataset.labelOriginalFontPriority = el.style.getPropertyPriority("font-size");
          el.style.setProperty("font-size", (computedSize * scale).toFixed(2) + "px", "important");
        });
      });
    }

    function scheduleApplyLabelTypography() {
      if (labelTypographyFrame) cancelAnimationFrame(labelTypographyFrame);
      labelTypographyFrame = requestAnimationFrame(function () {
        labelTypographyFrame = requestAnimationFrame(function () {
          labelTypographyFrame = null;
          applyLabelTypography();
        });
      });
    }
    ...
    function updateLogo(el) {
      if (!el) return;
      const hasLogo = !!systemSettings.logoData;
      el.hidden = !hasLogo;
      el.replaceChildren();
      if (el.parentElement) el.parentElement.classList.toggle("has-label-logo", hasLogo);
      if (!hasLogo) return;
    ...
    function updateLandscapePreview() {
      ...
      updateLogo(document.getElementById("label75-logo"));
    ...
    function updatePortraitPreview() {
      ...
      updateLogo(document.getElementById("preview-logo"));
    ...
    function updateMinumRsPreview() {
      ...
      [
        ...
        "mha-label-logo",
        "etiket-label-logo",
        "obat-luar-label-logo",
        "instruksi-label-logo"
      ].forEach((id) => updateLogo(document.getElementById(id)));
    ...
    function getSelectedLabelDimensions() {
      if (selectedPrintMode === "portrait") return portraitLabelSize;
      if (selectedPrintMode === "minum-rs") return minumRsLabelSize;
      return landscapeLabelSize;
    }

    function getActivePrintableRoot() {
      if (selectedPrintMode === "portrait") return document.getElementById("printable-area-portrait");
      if (selectedPrintMode === "minum-rs") return document.getElementById("printable-area-minum-rs");
      return document.getElementById("printable-area-landscape");
    }

    function getSelectedPrintModeName() {
      if (selectedPrintMode === "portrait") return "Label Potrait";
      if (selectedPrintMode === "landscape") return "Label Landcape";
      return getMinumSelectedLabelText();
    }

    function validateSelectedLabelForQueue() {
      if (!selectedPrintMode) return { ok: false, message: "Pilih dulu mode cetak label." };
      if (selectedPrintMode !== "minum-rs") {
        const instansiValidation = getInstansiValidation();
        if (!instansiValidation.ok) return instansiValidation;
      }
      if (selectedPrintMode === "minum-rs") {
        const labelValidation = getActiveLabelValidation();
        if (!labelValidation.ok) return labelValidation;
      }
      return { ok: true, message: "" };
    }

    function addCurrentLabelToQueue() {
      const validation = validateSelectedLabelForQueue();
      if (!validation.ok) return showPrintWarning(validation.message);
      clearPrintWarning();
      updateSizeVariables();
      updatePreview();
      ...
      requestAnimationFrame(function () {
        requestAnimationFrame(function () {
          if (selectedPrintMode === "minum-rs" && isCompactCustomLabel(getMinumLabelType()) && !fitCompactCustomLabel(getMinumLabelType())) {
            return showPrintWarning("Isi label terlalu panjang untuk ukuran 75 x 50 mm. Persingkat data sebelum menambahkannya ke antrian.");
          }

          const dimensions = getSelectedLabelDimensions();

          const source = getActivePrintableRoot();
          if (!source) return showPrintWarning("Preview label belum tersedia.");
          const clone = source.cloneNode(true);
          clone.removeAttribute("id");
          clone.querySelectorAll("[id]").forEach((el) => el.removeAttribute("id"));
          clone.classList.add("queue-print-label");
          clone.style.display = selectedPrintMode === "portrait" ? "flex" : "block";
          clone.style.width = dimensions.width + "mm";
          clone.style.height = dimensions.height + "mm";

          const data = getInputData();
          printQueueItems.push({
            id: String(createRecordId()),
            html: clone.outerHTML,
            mode: selectedPrintMode,
            labelName: getSelectedPrintModeName(),
            patientName: data.nama === "-" ? "Tanpa nama pasien" : data.nama,
            drugName: data.obat === "-" ? "" : data.obat,
            width: dimensions.width,
            height: dimensions.height
          });
          printQueueState.page = Math.max(1, Math.ceil(printQueueItems.length / PRINT_QUEUE_PAGE_SIZE));
          renderPrintQueue();
          showToast("Label ditambahkan ke antrian print.");
        });
      });
    }
    ...
    function renderPrintQueue() {
      ...
      const count = document.getElementById("print-queue-count");
      const printButton = document.getElementById("print-queue-btn");
      const clearButton = document.getElementById("clear-print-queue-btn");
      if (!list || !empty || !count || !printButton || !clearButton) return;

      count.textContent = printQueueItems.length + " label";
      empty.hidden = printQueueItems.length > 0;
      printButton.disabled = printQueueItems.length === 0;
      clearButton.disabled = printQueueItems.length === 0;

      // Pagination calculations
      const totalPages = Math.max(1, Math.ceil(printQueueItems.length / PRINT_QUEUE_PAGE_SIZE));
      printQueueState.page = Math.min(Math.max(printQueueState.page, 1), totalPages);
      const start = (printQueueState.page - 1) * PRINT_QUEUE_PAGE_SIZE;
      const visibleData = printQueueItems.slice(start, start + PRINT_QUEUE_PAGE_SIZE);

      const queueFooter = document.getElementById("print-queue-footer");

      if (printQueueItems.length === 0) {
        list.innerHTML = "";
        if (queueFooter) queueFooter.hidden = true;
      } else {
        if (queueFooter) queueFooter.hidden = false;

        // Render items (no checkboxes, only the label text/details and the red X delete button)
          ...
          return `
            <div class="print-queue-item">
              <div style="flex: 1; min-width: 0;">
                <strong>${absoluteIndex}. ${escapeHtml(item.labelName)} - ${escapeHtml(item.patientName)}</strong>
                <span>${escapeHtml(item.drugName || "Tanpa nama obat")} | ${item.width} x ${item.height} mm</span>
              </div>
              <button class="btn btn-danger btn-sm" type="button" onclick="removePrintQueueItem('${id}')" title="Hapus dari antrian"><i class="fa-solid fa-xmark"></i></button>
            </div>
          `;
        }).join("");

        // Render footer pagination controls
        updateQueueControls(visibleData, totalPages, start);
      }

      renderPrintQueueArea();
    }

    function updateQueueControls(visibleData, totalPages, start) {
      const info = document.getElementById("pagination-info-print-queue");
      if (info) {
        const end = Math.min(start + PRINT_QUEUE_PAGE_SIZE, printQueueItems.length);
        info.textContent = printQueueItems.length 
          ? `Menampilkan ${start + 1}-${end} dari ${printQueueItems.length} data` 
          : "0 data";
      }

      const atFirstPage = printQueueState.page === 1;
      const atLastPage = printQueueState.page === totalPages;
      const prevBtn = document.getElementById("pagination-prev-print-queue");
      const nextBtn = document.getElementById("pagination-next-print-queue");
      if (prevBtn) prevBtn.disabled = atFirstPage;
      if (nextBtn) nextBtn.disabled = atLastPage;

      const pagesSpan = document.getElementById("pagination-pages-print-queue");
      if (pagesSpan) pagesSpan.innerHTML = renderQueuePaginationButtons(totalPages);
    }

    function renderQueuePaginationButtons(totalPages) {
      const currentPage = printQueueState.page;
      const pages = new Set([1, totalPages]);
      ...
      return items.map((item) => item === "..."
        ? '<span class="pagination-ellipsis">...</span>'
        : `<button class="pagination-btn${item === currentPage ? " active" : ""}" type="button" onclick="setQueuePage(${item})">${item}</button>`
      ).join("");
    }

    function setQueuePage(page) {
      printQueueState.page = page;
      renderPrintQueue();
    }

    function changeQueuePage(direction) {
      const totalPages = Math.max(1, Math.ceil(printQueueItems.length / PRINT_QUEUE_PAGE_SIZE));
      if (direction === "prev") printQueueState.page = Math.max(1, printQueueState.page - 1);
      if (direction === "next") printQueueState.page = Math.min(totalPages, printQueueState.page + 1);
      renderPrintQueue();
    }

    function renderPrintQueueArea() {
      const area = document.getElementById("print-queue-print-area");
      if (!area) return;
      area.innerHTML = printQueueItems.map((item, index) => `
        <section class="queue-print-page queue-print-page-index-${index}" style="--queue-item-width:${item.width}mm;--queue-item-height:${item.height}mm;">
          ${item.html}
        </section>
      `).join("");
    }

    function removePrintQueueItem(id) {
      printQueueItems = printQueueItems.filter((item) => item.id !== String(id));
      
      // Adjust page if current page has no items left
      const totalPages = Math.max(1, Math.ceil(printQueueItems.length / PRINT_QUEUE_PAGE_SIZE));
      if (printQueueState.page > totalPages) printQueueState.page = totalPages;

      renderPrintQueue();
      showToast("Label dihapus dari antrian.");
    }

    async function clearPrintQueue() {
      if (!printQueueItems.length) return;
      const confirmed = await showConfirmDialog("Kosongkan seluruh antrian print?", {
        title: "Kosongkan Antrian",
        confirmText: "Kosongkan"
      });
      if (!confirmed) return;
      printQueueItems = [];
      printQueueState.page = 1;
      renderPrintQueue();
      showToast("Antrian print dikosongkan.");
    }

    function cleanupPrintQueueMode() {
      document.body.classList.remove("print-queue");
      document.documentElement.classList.remove("print-queue");
      const style = document.getElementById("print-queue-page-style");
      if (style) style.remove();
    }

    function printQueuedLabels() {
      if (!printQueueItems.length) return showPrintWarning("Tambahkan label ke antrian terlebih dahulu.");
      clearPrintWarning();
      renderPrintQueueArea();
      
      // Override default print queue width to dynamically scale with the page size
      document.documentElement.style.setProperty("--queue-label-width", "100%");
      
      let style = document.getElementById("print-queue-page-style");
      if (!style) {
        style = document.createElement("style");
        style.id = "print-queue-page-style";
        document.head.appendChild(style);
      }
      
      // Build dynamic named pages for each queue item to support multiple custom page sizes in one print job
      let styleContent = "@media print {\n";
      printQueueItems.forEach((item, index) => {
        styleContent += `  @page page-item-${index} { size: ${item.width}mm ${item.height}mm; margin: 0; }\n`;
        styleContent += `  .queue-print-page-index-${index} { page: page-item-${index} !important; }\n`;
      });
      styleContent += "}";
      style.textContent = styleContent;
      
      document.documentElement.classList.add("print-queue");
      document.body.classList.add("print-queue");
      setTimeout(function () {
        window.print();
        setTimeout(cleanupPrintQueueMode, 500);
      }, 120);
    }
    ...
    function getAuthUsers() {
      try {
        const raw = localStorage.getItem("isset/users");
    ...
    function saveAuthUsers(users) {
      localStorage.setItem("isset/users", JSON.stringify(users));
    }
    ...
    function renderAkunTable() {
      ...
      if (!users.length) {
        tbody.innerHTML = '<tr><td colspan="4" class="empty-state">Belum ada akun yang terdaftar.</td></tr>';
        return;
      }
      tbody.innerHTML = users.map((u) => {
        ...
        return `
          <tr>
            <td><strong>${escapeHtml(u.username)}</strong>${isCurrent ? ' <span class="badge" style="background:#059669;color:white;padding:2px 6px;border-radius:4px;font-size:0.75rem;">Saat ini</span>' : ''}</td>
            <td>${escapeHtml(u.nama || '-')}</td>
            <td>${date}</td>
            <td class="action-col-wide" style="text-align:right;">
              <button class="btn btn-danger btn-sm" type="button" onclick="deleteAkun('${escapeHtml(u.id)}', '${escapeHtml(u.username)}')" ${isCurrent ? 'disabled title="Tidak bisa menghapus akun yang sedang digunakan"' : 'title="Hapus Akun"'}><i class="fa-solid fa-trash"></i> Hapus</button>
            </td>
          </tr>
        `;
      }).join("");
    ...
    async function deleteAkun(userId, username) {
      if (currentUser.username === username) return;
      if (!await showConfirmDialog(`Hapus akun "${username}"? Akun tidak bisa dipulihkan kembali.`, {
        title: "Hapus Akun",
        confirmText: "Hapus"
      })) return;
